...
- 尽可能的使用 Composition API typescript 模式，非必要不使用tsx，严格禁止写成选项式 API。
...
- 根目录下的 `vendor` 文件夹默认视为“外部依赖的本地化产物”，里面的内容仅供参考，不要默认它们在当前任务里就是需要维护的输入或输出。
...
10. 是否存在 `components/index.scss` 这类根样式聚合入口需要同步。
11. 根样式聚合入口当前使用的 import 写法是什么；在本仓库中，现有 `@import url('./foo/style/index.scss');` 写法应视为有效约定，不要擅自去掉 `url()` 或改写路径分隔符。
