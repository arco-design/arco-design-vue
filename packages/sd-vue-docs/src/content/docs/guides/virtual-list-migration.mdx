...
当前组件库中除 `Table` 以外的虚拟列表能力，已经统一切换到 `vue-virtual-scroller`，并继续复用组件库内置的 `Scrollbar`，不再依赖系统滚动条。
...
<sd-tree
  :data="treeData"
  :virtual-list-props="{ height: 240, minItemSize: 32 }"
/>
```
...
`Table` 本次明确跳过，不切到新语义。相关旧 props 仍保留在旧实现兼容路径里，等后续重写 `Table` 时再重新定义。
