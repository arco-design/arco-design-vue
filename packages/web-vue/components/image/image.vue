...
<script setup lang="ts">
  ...
  const emit = defineEmits<{
     ...
     */
    'preview-visible-change': [_visible: boolean];
    'update:previewVisible': [_visible: boolean];
  }>();
  ...
  const mergePreview = computed(() => !groupContext?.preview && preview.value);
  ...
  defineExpose({ onImgLoaded, onImgLoadError, onImgClick, onPreviewClose });
</script>
