...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const nodeOptionValueMap = reactive(new Map<BaseType, string>());
    ...
    const computedRootValueMap = computed(() => {
      // 仅在需要时计算
        ...
        return new Map(
          values.map((value) => [
            getValueKey(value, {
              ...
              leafOptionValueMap: nodeOptionValueMap,
            }),
            value,
          ])
        );
    ...
    const selectViewValue = computed(() => {
      const result: any[] = [];
      (isShowParent.value
        ? computedRootValueMap.value
        : computedValueMap.value
      ).forEach((value, key) => {
        const option = (isShowParent.value ? optionMap : leafOptionMap).get(
          key
        );
        if (option) {
          result.push({
