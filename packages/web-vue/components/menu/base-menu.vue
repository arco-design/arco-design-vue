...
<script setup lang="ts">
  ...
  const emit = defineEmits<{
    ...
    'update:selectedKeys': [_selectedKeys: string[]];
    'update:openKeys': [_openKeys: string[]];
    /**
     * @zh 折叠状态改变时触发
     ...
     */
    'sub-menu-click': [_key: string, _openKeys: string[]];
  }>();
  ...
  const computedStyle = computed(() => {
    ...
    const objectStyle = isObject(style!.value) ? (style!.value as CSSProperties) : undefined;
