...
<script lang="ts">
  ...
  const SIDER_LOCATIONS = ['start', 'end', 'left', 'right', 'top', 'bottom'] as const;
  ...
  export default defineComponent({
    ...
    emits: ['update:modelValue', 'update:rail', 'collapse', 'update:collapsed', 'breakpoint'],
    /**
     * @zh 自定义底部折叠触发器
    ...
    setup(props, { emit }) {
      ...
      const railModeEnabled = computed(
        () => (rail.value ?? defaultRail.value) || railCollapsed.value,
      );
      ...
      const siderStyles = computed(() => {
        const style = isHorizontal.value
          ? { width: siderSize.value }
