...
从当前脚本、构建配置和包边界来看，这个前端项目不是“组件库 + 文档站”的简单并列关系，而是一个以 `packages/web-vue` 为核心产物、以 `packages/sd-vue-docs` 为消费与展示层的双包协同架构。

- 根目录负责统一调度开发、构建、校验和发布前检查。
- `packages/web-vue` 负责组件源码、样式、类型声明和 IDE 元数据产出。
- `packages/sd-vue-docs` 负责文档页面、在线示例、主题桥接和站点构建。

可以把它理解成一条完整的前端交付链，而不是两个互不相关的前端子项目。
...
这意味着文档站问题很多时候不是单点问题，而是“文档配置 + 组件库产物 + 运行时桥接”三者之一出了偏差。
...
flowchart TD
	...
	Parallel[并行启动开发服务]
	WatchBuild[组件库 watch 构建\n@sdata/web-vue dev:serve]
	DocsDev[文档站开发服务\n@sd-design/sd-vue-docs dev:serve]
