...
- [ ] 3.4 验证组件级 token override 优先级（global -> component）。
- [ ] 3.5 对每批迁移执行视觉回归和快照校验并修复问题。

## 4. Documentation Theme Editor

- [ ] 4.1 在 `packages/sd-vue-docs` 新增主题编辑器路由与页面骨架。
- [ ] 4.2 实现编辑器参数面板（颜色、圆角、字号、间距等）与节流更新策略。
- [ ] 4.3 构建实时预览容器，使用 ConfigProvider `theme` 对象驱动示例组件。
- [ ] 4.4 实现主题 JSON 下载（含 schemaVersion）与上传解析流程。
- [ ] 4.5 增加 JSON schema 兼容性校验与错误提示（不兼容版本、缺失字段）。

## 5. Homepage Theme Showcase

- [ ] 5.1 在首页新增主题切换展示模块并接入预设主题集合。
- [ ] 5.2 提供至少默认/暗色/紧凑/品牌色/赛博朋克风等示例主题预览。
- [ ] 5.3 复用编辑器与组件库统一主题协议，确保预设对象可直接传给 ConfigProvider。
- [ ] 5.4 为首页主题切换交互补充文档示例与回归测试。

## 6. Validation, Migration Docs, and Cleanup

- [ ] 6.1 执行组件库与文档站全量构建与测试，修复迁移引入的问题。
- [ ] 6.2 编写迁移文档（Less -> Scss、主题对象用法、CSS variables 约定）。
- [x] 6.3 标记并清理废弃 Less 变量链路与临时兼容代码。
- [ ] 6.4 在“开始使用”的下面新增一个<迁移文档>分类，把“Naive UI 迁移”放到这个分类下面，同时输出本次样式修改的手册（迁移步骤与风险说明）。
